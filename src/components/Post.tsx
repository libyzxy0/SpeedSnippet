...
function Post({ children, className, post }: PostProps) {
  ...
  const handleChangeReaction = (reactions: Reaction[]) => {
    ...
    const crtext =
      total == 0
        ? ""
        : `${most} people says its ${isAwesome ? "Awesome ⚡" : "Trash 💩"}`;
    setRText(crtext);
  };
    ...
    if (window.location.hostname) {
      try {
        await navigator.clipboard.writeText(
          `${window.location.protocol}//${window.location.host}/post/${post.id}?from=copylink`,
        );
      } catch (err) {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-56 px-2">
              ...
              <DropdownMenuGroup>
                <DropdownMenuItem
                  onClick={handleCopy}
                  className="py-2 my-2 hover:bg-gray-800"
                >
                  Copy link
                ...
                <DropdownMenuItem
                  className="py-2 my-2 hover:bg-gray-800"
                  onClick={() => navigate(`/post/${post.id}?from=explore`)}
                >
                  View Post
                </DropdownMenuItem>
              </DropdownMenuGroup>
              <DropdownMenuSeparator />
              <DropdownMenuItem className="py-2 my-2 hover:bg-gray-800">
                Report abuse
              </DropdownMenuItem>
              {authenticated && authenticated?.id == user_id ? (
                <>
                  ...
                  <DropdownMenuItem className="bg-sky-400 my-2 text-white hover:bg-sky-500">
                    Edit Post
                  </DropdownMenuItem>
                  <DropdownMenuItem
...
function Caption() {
  ...
  return (
    <div
      onClick={() => navigate(`/post/${post.id}?from=explore`)}
      className="border-t border-gray-300 dark:border-gray-800 flex flex-col"
    >
...
function CodeSnippet() {
  ...
  return (
    <div className="px-4 pt-4 flex flex-col">
      ...
      <div
        className={`p-1.5 rounded-bl-md rounded-br-md ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}
      >
        <SyntaxHighlighter
...
function Reaction() {
  ...
  useEffect(() => {
    ...
    if (currentPost) {
      const userReaction = currentPost.reactions.find(
  ...
  return (
    <div className="border-t border-t-gray-300 dark:border-t-gray-800 border-b-4 border-b-sky-300 dark:border-b-sky-300 flex flex-row mt-0">
      <Button
        onClick={() => handleReaction(id, "awesome")}
        className={`bg-white shadow-none rounded-none dark:bg-gray-950 py-6 border-none w-[50%] transition-all duration-200 hover:bg-transparent ${cReaction === "awesome" ? "text-sky-400" : "text-gray-400 hover:text-gray-900 dark:hover:text-white"}`}
      >
        ⚡ Awesome
      </Button>
      <Button
        onClick={() => handleReaction(id, "trash")}
        className={`bg-white shadow-none rounded-none dark:bg-gray-950 py-6 border-none w-[50%] transition-all duration-200 hover:bg-transparent ${cReaction === "trash" ? "text-sky-400" : "text-gray-400 hover:text-gray-900 dark:hover:text-white"}`}
      >
        💩 Trash
      </Button>
    </div>
